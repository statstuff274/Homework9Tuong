...
> Now we'll pull out the best model using select_best() and finalize_workflow
...
> We obtained our best LASSO model with a penalty of 0.0000000001, or 1e-10. Now we're going to finish our training.
...
> Now we'll sort by the smallest rmse value and select_best to grab the best model's tuning parameter values. We end up with a value of 0.001. Then we'll finalize our model.
...
> Now we're going to obtain our best tuning parameter, which is 0.00000349. We're complete our workflow from our tuning and then finalize the model to be fit to bike_train.
...
> Now we're going to tune the random forest model!
...
> Here I'll have compiled the best (lowest rmse) models for each model.
...
# Regression Tree best rmse
reg_best
```

> Notes penalty = tune(); We're going to be using a resampling model - i.e. CV.
